...
- 一般論的な分析よりも、**プロジェクト固有の具体的な環境・実績に即した分析**を優先する。
- 「一般的に有効とされている」のような記述ではなく、実際のリソース・制約条件（予算・機材・人員・期間）を参照した提案をする。

### 変更影響分析（R5/R12）

- 既存ファイルに波及する変更（スキーマ変更、名称変更など）を行う場合:
  1. **影響範囲を先にリスト化**する（grep 等で関連ファイルを検索）
  2. ユーザに影響範囲を提示（ファイル数が多い場合）
  3. リスト化した全ファイルに対して修正を実行
  4. 修正漏れがないことを検証
- **影響ファイル一覧（事前）** は `### 対応` セクションに記録すること（R12）
  - 形式: 修正対象ファイルパスのリスト + 変更概要
  - 事前の見積りと事後の実績が異なる場合はその旨も記録する

### 未検証主張の取り扱い（R13）

- **原則**: プロジェクト開始前（計画・申請段階）の評価・分析は**基本的に未検証**である
- 検証済みの主張には **「検証済み」** ラベルを明示的に付与する
  - 最低限の根拠: テスト結果 / 再現実験 / 定量データ / 外部文献の引用
- 未検証のまま意思決定に使用された評価は、後続イシューで検証機会が生じた際に更新する

### ツール推奨時の実地検証（R18）

- 外部ツール・サービスの導入を推奨する場合は、最低限のハンズオン検証（インストール確認・基本動作テスト等）を行うか、未検証である旨を明記する
