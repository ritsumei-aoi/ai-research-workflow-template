# AI ワークフロー — Method C（GitHub Agentic ワークフロー）[ドラフト/提案]

> **ステータス: ドラフト/提案** — 本ドキュメントは構想段階であり、まだ運用に入っていない。
> 実運用開始時にこのバナーを除去し、workflow_common.md の方式一覧テーブルに追加すること。
...
|---|---|---|---|
| 実行主体 | 人間（AI の助言を受けて適用） | AI エージェント（ローカル環境） | GitHub Agentic ワークフロー（クラウド） |
...
### ステップ 2. Agentic ワークフローのトリガー

GitHub Agentic ワークフローを Issue または PR コメントから起動する。

- Agentic ワークフローが Issue の内容に基づいて PR を自動生成
...
- スカッシュマージを使用（[workflow_common.md](workflow_common.md) のブランチ規則に準拠）
- コミットメッセージに Issue 番号を含める: `#<issue-number>: [要約]`
- マージ後、Issue を自動クローズ
...
1. **コンテキストの欠如**: Agentic ワークフローはセッション間のコンテキストを持たない。
