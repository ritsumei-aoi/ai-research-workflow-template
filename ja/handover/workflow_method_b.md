...
**推奨LLMモデルの選択**: `issue_open.md` 作成時、使用しないモデルの選択肢（`[ ]` が残る行）は**削除**してください。これは `close_issue.sh` の自動チェック（未完了の `[ ]` の検出）によるエラーを防止するためです。
...
1. `issue_open.md` の完了条件チェックボックスを `[x]` に更新し、各項目に `### 対応` セクションを追記
   - **使用モデルの記録**: `### 対応` セクションの冒頭に、実際に使用したモデル名（例: `Gemini 3.1 Pro (Auto)`）を記載してください。
