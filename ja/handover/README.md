...
- [引き継ぎメモフォーマット](handover_memo_format.md)
  - `handover_memo_latest.md` / `handover_memo_archived.md` のテンプレート定義。
- [引き継ぎメモ（最新）](handover_memo_latest.md)
  - 最新セッションの進捗、次回への引き継ぎ、直近の注意点。
- [引き継ぎメモ（アーカイブ）](handover_memo_archived.md)
  - 過去セッションの履歴（時系列、追記型）。

## 2. 実装仕様・設計資料

<!-- CUSTOMIZE: プロジェクト固有の実装文書をここに追加。例:
...
<!-- CUSTOMIZE: プロジェクト固有の理論文書をここに追加。 -->

## 4. 参照・調査資料

<!-- CUSTOMIZE: プロジェクト固有の参考資料をここに追加。 -->

## 5. サンプル

完了済みイシューのサンプルは `/samples/docs/issues/done/` にあります。
英語・日本語の両方でイシューライフサイクルの全体像を示しています。

理論資料のサンプルは `/samples/docs/theory/` にあります。
`docs/theory/` にイシューや検証スクリプトから参照される
数学的背景をどのように格納するかを示しています。
