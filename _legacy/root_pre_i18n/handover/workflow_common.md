...
- AI 委任境界（[ai_trust_policy.md](ai_trust_policy.md)）を常に遵守する。

## Branching Policy

1. Method B: `ai/workflow_issue` 固定ブランチを使用。
2. Method A: `ai/<YYYY-MM-DD>-<topic>` で日付トピックブランチ。
3. Method C: `agentic/<issue>-<topic>` で Issue ベースブランチ。
...
- [ ] Output 読み込み確認 (Method B required protocol)
