...
- **数学的理論の検証**: Lie 超代数の構造定数の正当性、証明の論理的整合性
  → Method A（対話型）で人間が逐次確認
- **新規証明の構成**: 余境界作用素 δ の性質、コホモロジー計算、certificate 構成
  → Method A/B で人間が深く関与
- **スキーマ設計の意思決定**: `schema_version` の変更を伴う構造変更
  → Method A で設計議論 → Method B で実装
- **理論と実装の対応付け**: `theory_vs_implementation.md` に関わる変更
  → Method A/B で理論的文脈を保持しながら作業
...
1. **文脈の欠如**: Agentic Workflow はセッション間の文脈を持たない。
   `handover_memo_latest.md` や `notation.md` の暗黙知に依存するタスクには不向き。

2. **数学的正しさの保証困難**: 自動生成されたコードが数学的に正しいかどうかは、
   テストだけでは判断できない場合がある（例: 構造定数の符号、√2 の扱い）。

3. **理論ドキュメントとの整合**: `docs/theory/` 配下の導出ノートとの整合性は
   自動検証が困難。人間のレビューが必須。

4. **Schema 依存**: `schema_version` に影響する変更は、
   データファイル（`data/`）との整合性確認が必要。Method C 単独では危険。
