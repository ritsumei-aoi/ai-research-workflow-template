...
## 3. 具体的なタスク別ガイド（Lie 超代数研究プロジェクト）

### Method A を使うべきタスク

| タスク例 | 理由 |
|---|---|
| B(m,n) の構造定数の数学的検証 | γ 構造定数の符号・係数の正しさは対話的に確認が必要 |
| 余境界作用素 δ の性質の証明 | 証明の各ステップで人間の判断が不可欠 |
| 新しい certificate family の発見 | 探索的な作業であり、方向性の判断に対話が必要 |
| Schema 設計の意思決定 | `schema_version` 変更は広範な影響があり、対話で合意形成 |
| 論文草稿のレビュー・修正 | 数学的表現の適切さは人間の判断が必要 |
| `theory_vs_implementation.md` の更新 | 理論と実装の対応付けには深い文脈理解が必要 |
| `notation.md` の規約変更 | 記法変更は全体に波及するため、慎重な議論が必要 |

### Method B を使うべきタスク

| タスク例 | 理由 |
|---|---|
| 新しい代数 B(m,n) の実装 | コード生成＋テスト実行を一貫して実施 |
| Phase 1-3 の計算的検証スクリプト | 実行結果を即座に確認しながら開発 |
| Schema 3/4 のパーサー・ビルダー実装 | 既存コードとの整合性を確認しつつ実装 |
| `data/` 配下の JSON 生成・検証 | ファイル生成→パース→検証の一連の流れ |
| テストの追加・修正 | 既存テストとの整合性を確認しながら追加 |
| handover ドキュメントの更新 | セッションコンテキストに基づく更新 |
| `experiments/` スクリプトの新規作成 | 実行結果を確認しながらイテレーション |
...
| import 文の整理・ソート | 機械的な変更、テストで検証可能 |
| docstring の追加・フォーマット統一 | テンプレート的な作業 |
| 非推奨 API の置換 | パターン置換、テストで検証可能 |
| GitHub Actions ワークフローの追加 | CI/CD は独立して検証可能 |
| `requirements.txt` の依存更新 | テスト通過で互換性を確認 |
| lint 警告の修正 | 機械的な修正 |
| テストのパラメータ化リファクタリング | テスト構造の改善、既存テストで検証 |
...
例: 新しい B(m,n) の triviality 証明
  1. Method A: 証明戦略の議論、certificate 構成の方針決定
...
本プロジェクトは Lie 超代数の研究リポジトリであり、コードの正しさ＝数学的正しさである場面が多い。
